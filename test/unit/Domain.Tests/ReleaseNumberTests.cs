...
public class ReleaseNumberTests
{
    private static IEnumerable<object[]> ReleaseNumbers() => new List<object[]>
    {
        ...
        new object[] { 230, 9, 230, 9, true }
    };
        
    [Theory]
    [MemberData(nameof(ReleaseNumbers))]
    public void Determines_if_ReleaseNumber_is_newer_or_equal_to_other_ReleaseNumber(int major, int minor, int majorOther, int minorOther, bool expected)
    {
        var releaseNumber = ReleaseNumber.Create(major, minor);
        var releaseNumberOther = ReleaseNumber.Create(majorOther, minorOther);

        var isOtherNewer = releaseNumber.Value.IsNewerOrEqualTo(releaseNumberOther.Value);
